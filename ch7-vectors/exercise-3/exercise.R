...
words <- c("This", "is", "not", "a", "great", "test")
...
# of the word!
a_f_words <- words[tolower(words) >= "a" & tolower(words)  <= "f" ]

# Create a vector `g_m_words` which are the elements in `words` that start with 
# "g" through "m"
g_m_words <- words[tolower(words) >= "g" & tolower(words)  <= "m" ]
...
word_bin <- function(words, start, end){
  result <- words[tolower(words) >= start & tolower(words)  <= end ]
  result
}
